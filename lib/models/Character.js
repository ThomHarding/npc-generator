...
async function getWashName() {
  ...
  const rngFirst = Math.round(Math.random() * 9) + 1;
  const rngLast = Math.round(Math.random() * 9) + 1;
  const { rows } = await pool.query(
...
async function getHofangName() {
  ...
  console.log('here we go again', rngFirst, rngLast, rows2.rows);
...
function generateAge() {
  const rngAge = Math.round(Math.random() * 9) + 1;
  if (rngAge === 0) {
    return 1;
  } else if (rngAge > 0 <= 2) {
    return 2;
