...
class Character {
  ...
  static async testInsert(name, region_id) {
    ...
    const species = generateSpecies(regionInfo[0]);
    ...
    //so now it just doesn't insert. man at least it's trying. pogress
...
function generateSpecies(regionInfo) {
  regionInfo; //now stop givin me errors
  //TODO: some fucked up math
  return 1;
  ...
  //   if (rng >= species[i]) {
  //     return 'human';//TODO the name of the row? wait how do i do that
  //   } else {
  //     return 'shapeshifting wizard';
  //   }
  // }
}
